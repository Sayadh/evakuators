...
| Change how often a visitor may run the nearest search | `docs/nearest-search.md` § "How often one person may search" — there are **two** daily numbers and they are deliberately far apart: 2/day per browser is the product rule with visible copy, 40/day per IP is an abuse ceiling. Do not make them match — `req.ip` is not a person (Armenian carriers use CGNAT), so a per-IP 2 would refuse the third phone on a carrier address. Both charge for *searches performed*, never requests received, so a cache hit is free and is still served to someone over the ceiling. The browser stores the **answer** and a counter, never the coordinates |
