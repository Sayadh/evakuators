...
/**
 * `findAllForAdmin`'s `vehicleType` param is plain equality on the raw
 * column — deliberately NOT the manipulator/heavy-duty union `buildWhere`
 * applies for the public `/tow-trucks` listing. See `AdminTowTrucksQuery`'s
 * own comment for why: every admin card already shows that truck's own
 * `vehicleType` label, so pulling in a flatbed-with-a-crane under a
 * "Մանիպուլյատոր" filter would show a card whose own label disagrees with
 * the filter that surfaced it.
 *
...
describe('TowTrucksRepository.findAllForAdmin', () => {
  it('sends no where clause when no vehicleType is given', async () => {
  ...
  it('filters by plain equality when a vehicleType is given', async () => {
    ...
    await new TowTrucksRepository(prisma).findAllForAdmin({ limit: 50, offset: 0 }, 'manipulator')
  ...
  it('passes limit/offset through unchanged, same as before this filter existed', async () => {
    ...
    await new TowTrucksRepository(prisma).findAllForAdmin({ limit: 20, offset: 40 }, 'heavy-duty')
