...
| `GET` | `/admin/tow-trucks/broadcast-candidates` | Read-only — active, Telegram-linked drivers the admin broadcast can currently reach. The panel lists these with checkboxes so recipients are chosen before anything is sent |
| `POST` | `/admin/tow-trucks/broadcast-message` | Body `{ message: string, towTruckIds: number[] }` — sends `message` verbatim, no button, to **exactly** the drivers named — required and non-empty, there is no "send to everyone" shorthand, because a Telegram message cannot be unsent and staging's database holds real drivers' real chat ids. `message` capped at `TELEGRAM_MESSAGE_MAX_LENGTH` (4000, both sides — see CLAUDE.md § "Manual sync points"), comfortably under Telegram's own 4096-character `sendMessage` limit. `towTruckIds` intersected with the live active+linked candidate list. `{ sent, failed: [{ id, slug }], skipped }`. See `docs/auth-and-security.md` § "The admin broadcast" |
| `GET` | `/admin/tow-trucks/export.csv` | Every driver, active and deactivated, as an Excel-safe CSV (UTF-8 BOM, CRLF). Six columns and no more — «Անուն Ազգանուն», «Հեռախոս», «Զանգեր (ընդամենը)» (`PHONE_CLICK` only, not every contact tap), «Ուղղորդումներ (ընդամենը)» (`DispatchReferral` rows), «Հիմնական գտնվելու վայրը» (`locationName`, composed on the frontend when the base was set) and «Մեր վարորդը» (`isPartner`, Այո/Ոչ). Both counters are all-time: the sheet carries no period selector, so a windowed number would be uninterpretable a week later. Columns live in `analytics/admin-drivers-export.rows.ts` |
