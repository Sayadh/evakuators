...
| `POST` | `/nearest-tow-trucks` | 10/60s | Body `{ latitude, longitude }` — the visitor's own position. Returns `{ results, routed }`, at most 10 drivers, each an ordinary **card shape** plus `straightLineMeters` and (when routing succeeded) `roadMeters`/`durationSeconds`. A **POST** despite being a read: a query string would put the visitor's exact coordinates in nginx's `access.log` next to their IP. The position is never stored — see `docs/nearest-search.md` |
...
| `POST` | `/admin/tow-trucks/issue-passwords` | One-time migration action — sends a password directly to every driver who already has `telegramChatId` set but no password, without asking them to re-link. Idempotent (already-migrated/self-changed drivers are silently skipped), so safe to press again. `{ issued, failed: [{ id, slug }] }` |
